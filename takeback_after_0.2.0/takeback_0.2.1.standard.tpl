...
var converttohtml = function (file) {
	var filename = file.split('/').pop();
	var path = file.match(/\/(.+\/)*/)[0];
	notice('{.!Converting.} '+filename+' {.!to .html format..} {.!Please wait.}', '{.!Conversion Started.}');
	var xhr = new XMLHttpRequest();
	xhr.open('POST', '/~ajax.convertdoctohtml?file='+filename+'&path='+path);
